...
def branch(puzzle_matrix, state, deadlock_patterns, transposition_table):
    ...
    for i in range(len(state[1])):
        for pushable in pushable_squares(puzzle_matrix, reachables, state[1][i], state[1]):
            ...
            new_reachables = reachable_squares(puzzle_matrix, state[1][i], new_stones)
            child_state = get_state(new_reachables, new_stones)
            child_hash_test = str(hash(child_state))
            # if child_hash_test == "4567526257665680067":
            #     draw_on_static_puzzle(puzzle_matrix, child_state, [state[0]])
            #     print("Player is at ", child_state[0])
            #     input(f"Pausing at {child_hash_test}")
            #print(child_hash_test)

            #draw_many_on_static_puzzle(puzzle_matrix, [child_state])
            #input("-")
