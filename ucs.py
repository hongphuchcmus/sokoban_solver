...
class UCSSolver:
    ...
    def branch(self, g : skb.Sokoban, state: skb.State) -> tuple[int, list[skb.State]]:
        ...
        if not self.use_ares_steps:
            costs_to_reachables = self.costs_to_reachable_square(g, state.ares_position, state.stone_positions)
            for push_position in costs_to_reachables:
                for stone_id in range(len(state.stone_positions)):
                    ...
                    push_cost = costs_to_reachables[push_position]
                    # Combine the two to get the total weight
                    weight = stone_weight + push_cost
